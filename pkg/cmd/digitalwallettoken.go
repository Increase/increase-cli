...
var digitalWalletTokensRetrieve = cli.Command{
	Name:    "retrieve",
	Usage:   "Retrieve a Digital Wallet Token",
	Suggest: true,
	Flags: []cli.Flag{
...
var digitalWalletTokensList = requestflag.WithInnerFlags(cli.Command{
	Name:    "list",
	Usage:   "List Digital Wallet Tokens",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:      "card-id",
			Usage:     "Filter Digital Wallet Tokens to ones belonging to the specified Card.",
			QueryPath: "card_id",
		},
	...
	Action:          handleDigitalWalletTokensList,
	HideHelpCommand: true,
...
func handleDigitalWalletTokensRetrieve(ctx context.Context, cmd *cli.Command) error {
	client := increase.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("digital-wallet-token-id") && len(unusedArgs) > 0 {
		cmd.Set("digital-wallet-token-id", unusedArgs[0])
	...
	_, err = client.DigitalWalletTokens.Get(ctx, cmd.Value("digital-wallet-token-id").(string), options...)
	if err != nil {
		return err
	}
...
func handleDigitalWalletTokensList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		var res []byte
		options = append(options, option.WithResponseBodyInto(&res))
		_, err = client.DigitalWalletTokens.List(ctx, params, options...)
		if err != nil {
			return err
		}
		obj := gjson.ParseBytes(res)
