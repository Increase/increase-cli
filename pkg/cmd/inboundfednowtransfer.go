...
var inboundFednowTransfersRetrieve = cli.Command{
	Name:    "retrieve",
	Usage:   "Retrieve an Inbound FedNow Transfer",
	Suggest: true,
	Flags: []cli.Flag{
...
var inboundFednowTransfersList = requestflag.WithInnerFlags(cli.Command{
	Name:    "list",
	Usage:   "List Inbound FedNow Transfers",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:      "account-id",
			Usage:     "Filter Inbound FedNow Transfers to those belonging to the specified Account.",
			QueryPath: "account_id",
		},
		&requestflag.Flag[string]{
			Name:      "account-number-id",
			Usage:     "Filter Inbound FedNow Transfers to ones belonging to the specified Account Number.",
	...
	Action:          handleInboundFednowTransfersList,
	HideHelpCommand: true,
...
func handleInboundFednowTransfersRetrieve(ctx context.Context, cmd *cli.Command) error {
	client := increase.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("inbound-fednow-transfer-id") && len(unusedArgs) > 0 {
		cmd.Set("inbound-fednow-transfer-id", unusedArgs[0])
	...
	_, err = client.InboundFednowTransfers.Get(ctx, cmd.Value("inbound-fednow-transfer-id").(string), options...)
	if err != nil {
		return err
	}
...
func handleInboundFednowTransfersList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		var res []byte
		options = append(options, option.WithResponseBodyInto(&res))
		_, err = client.InboundFednowTransfers.List(ctx, params, options...)
		if err != nil {
			return err
		}
		obj := gjson.ParseBytes(res)
