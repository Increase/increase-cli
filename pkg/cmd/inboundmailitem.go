...
func handleInboundMailItemsRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	return ShowJSON(os.Stdout, os.Stderr, "inbound-mail-items retrieve", obj, format, explicitFormat, transform)
}
...
func handleInboundMailItemsList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		...
		obj := gjson.ParseBytes(res)
		return ShowJSON(os.Stdout, os.Stderr, "inbound-mail-items list", obj, format, explicitFormat, transform)
	} else {
		...
		return ShowJSONIterator(os.Stdout, os.Stderr, "inbound-mail-items list", iter, format, explicitFormat, transform, maxItems)
	}
}
...
func handleInboundMailItemsAction(ctx context.Context, cmd *cli.Command) error {
	...
	explicitFormat := cmd.Root().IsSet("format")
	transform := cmd.Root().String("transform")
	return ShowJSON(os.Stdout, os.Stderr, "inbound-mail-items action", obj, format, explicitFormat, transform)
}
