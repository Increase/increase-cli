...
func handleInboundMailItemsRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	return ShowJSON(os.Stdout, "inbound-mail-items retrieve", obj, format, transform)
}
...
func handleInboundMailItemsList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		...
		obj := gjson.ParseBytes(res)
		return ShowJSON(os.Stdout, "inbound-mail-items list", obj, format, transform)
	} else {
		...
		return ShowJSONIterator(os.Stdout, "inbound-mail-items list", iter, format, transform, maxItems)
	}
}
...
func handleInboundMailItemsAction(ctx context.Context, cmd *cli.Command) error {
	...
	format := cmd.Root().String("format")
	transform := cmd.Root().String("transform")
	return ShowJSON(os.Stdout, "inbound-mail-items action", obj, format, transform)
}
