...
var cardPurchaseSupplementsRetrieve = cli.Command{
	Name:    "retrieve",
	Usage:   "Retrieve a Card Purchase Supplement",
	Suggest: true,
	Flags: []cli.Flag{
...
var cardPurchaseSupplementsList = requestflag.WithInnerFlags(cli.Command{
	Name:    "list",
	Usage:   "List Card Purchase Supplements",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:      "card-payment-id",
			Usage:     "Filter Card Purchase Supplements to ones belonging to the specified Card Payment.",
			QueryPath: "card_payment_id",
		},
	...
	Action:          handleCardPurchaseSupplementsList,
	HideHelpCommand: true,
...
func handleCardPurchaseSupplementsRetrieve(ctx context.Context, cmd *cli.Command) error {
	client := increase.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("card-purchase-supplement-id") && len(unusedArgs) > 0 {
		cmd.Set("card-purchase-supplement-id", unusedArgs[0])
	...
	_, err = client.CardPurchaseSupplements.Get(ctx, cmd.Value("card-purchase-supplement-id").(string), options...)
	if err != nil {
		return err
	}
...
func handleCardPurchaseSupplementsList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		var res []byte
		options = append(options, option.WithResponseBodyInto(&res))
		_, err = client.CardPurchaseSupplements.List(ctx, params, options...)
		if err != nil {
			return err
		}
		obj := gjson.ParseBytes(res)
