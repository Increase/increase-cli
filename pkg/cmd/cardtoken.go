...
var cardTokensRetrieve = cli.Command{
	Name:    "retrieve",
	Usage:   "Retrieve a Card Token",
	Suggest: true,
	Flags: []cli.Flag{
...
var cardTokensList = requestflag.WithInnerFlags(cli.Command{
	Name:    "list",
	Usage:   "List Card Tokens",
	Suggest: true,
	Flags: []cli.Flag{
...
func handleCardTokensRetrieve(ctx context.Context, cmd *cli.Command) error {
	client := increase.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("card-token-id") && len(unusedArgs) > 0 {
		cmd.Set("card-token-id", unusedArgs[0])
	...
	_, err = client.CardTokens.Get(ctx, cmd.Value("card-token-id").(string), options...)
	if err != nil {
		return err
	}
...
func handleCardTokensList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		var res []byte
		options = append(options, option.WithResponseBodyInto(&res))
		_, err = client.CardTokens.List(ctx, params, options...)
		if err != nil {
			return err
		}
		obj := gjson.ParseBytes(res)
...
func handleCardTokensCapabilities(ctx context.Context, cmd *cli.Command) error {
	client := increase.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("card-token-id") && len(unusedArgs) > 0 {
		cmd.Set("card-token-id", unusedArgs[0])
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.CardTokens.Capabilities(ctx, cmd.Value("card-token-id").(string), options...)
	if err != nil {
		return err
	}
	...
	return ShowJSON(obj, ShowJSONOpts{
		ExplicitFormat: explicitFormat,
		Format:         format,
		Title:          "card-tokens capabilities",
		Transform:      transform,
	})
}
