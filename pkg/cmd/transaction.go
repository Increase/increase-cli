...
var transactionsList = requestflag.WithInnerFlags(cli.Command{
	Name:    "list",
	...
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:      "account-id",
			Usage:     "Filter Transactions for those belonging to the specified Account.",
		...
		&requestflag.Flag[string]{
			Name:      "route-id",
			Usage:     "Filter Transactions for those belonging to the specified route. This could be a Card ID or an Account Number ID.",
			QueryPath: "route_id",
		},
		&requestflag.Flag[int64]{
			Name:  "max-items",
...
func handleTransactionsRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	return ShowJSON(os.Stdout, os.Stderr, "transactions retrieve", obj, format, explicitFormat, transform)
}
