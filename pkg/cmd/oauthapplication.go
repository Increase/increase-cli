...
var oauthApplicationsRetrieve = cli.Command{
	Name:    "retrieve",
	Usage:   "Retrieve an OAuth Application",
	Suggest: true,
	Flags: []cli.Flag{
...
var oauthApplicationsList = requestflag.WithInnerFlags(cli.Command{
	Name:    "list",
	Usage:   "List OAuth Applications",
	Suggest: true,
	Flags: []cli.Flag{
	...
	Action:          handleOAuthApplicationsList,
	HideHelpCommand: true,
...
func handleOAuthApplicationsRetrieve(ctx context.Context, cmd *cli.Command) error {
	client := increase.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("oauth-application-id") && len(unusedArgs) > 0 {
		cmd.Set("oauth-application-id", unusedArgs[0])
	...
	_, err = client.OAuthApplications.Get(ctx, cmd.Value("oauth-application-id").(string), options...)
	if err != nil {
		return err
	}
...
func handleOAuthApplicationsList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		var res []byte
		options = append(options, option.WithResponseBodyInto(&res))
		_, err = client.OAuthApplications.List(ctx, params, options...)
		if err != nil {
			return err
		}
		obj := gjson.ParseBytes(res)
