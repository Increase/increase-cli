...
var cardPaymentsRetrieve = cli.Command{
	Name:    "retrieve",
	Usage:   "Retrieve a Card Payment",
	Suggest: true,
	Flags: []cli.Flag{
...
var cardPaymentsList = requestflag.WithInnerFlags(cli.Command{
	Name:    "list",
	Usage:   "List Card Payments",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:      "account-id",
			Usage:     "Filter Card Payments to ones belonging to the specified Account.",
			QueryPath: "account_id",
		},
		&requestflag.Flag[string]{
			Name:      "card-id",
			Usage:     "Filter Card Payments to ones belonging to the specified Card.",
			QueryPath: "card_id",
		},
...
func handleCardPaymentsRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	_, err = client.CardPayments.Get(ctx, cmd.Value("card-payment-id").(string), options...)
	if err != nil {
		return err
	}
...
func handleCardPaymentsList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		var res []byte
		options = append(options, option.WithResponseBodyInto(&res))
		_, err = client.CardPayments.List(ctx, params, options...)
		if err != nil {
			return err
		}
		obj := gjson.ParseBytes(res)
